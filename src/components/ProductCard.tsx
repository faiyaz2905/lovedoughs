    ...
    >
      <div className="relative aspect-[4/5] overflow-hidden bg-blush">
        ...
        <motion.div
          ...
          animate={hover ? { y: 0, opacity: 1, scale: 1 } : { y: 20, opacity: 0, scale: 0.9 }}
      ...
      <div className="relative p-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="relative shrink-0 text-right">
            ...
            <Sparkle
              className={`absolute -right-3 -top-4 h-5 w-5 text-gold transition-all duration-500 ${hover ? "scale-100 rotate-12 opacity-100" : "scale-50 opacity-0"}`}
            />
          </div>
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {product.flavorTags.map((tag) => (
            <span
              key={tag}
              className="rounded-full bg-blush px-3 py-1 font-body text-xs font-medium text-chocolate"
            >
              {tag}
            </span>
          ))}
        ...
        <HeartDoodle
          className={`absolute -right-2 -top-2 h-8 w-8 text-gold transition-all duration-700 ${hover ? "scale-100 opacity-100 -rotate-12" : "scale-0 opacity-0"}`}
        />
      </div>
    </Link>
  );
